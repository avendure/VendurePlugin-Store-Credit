...
@Component({
    ...
    template: `
        <vdr-card style="margin-top: 1rem;" [paddingX]="false">
            <vdr-action-bar>
                <vdr-ab-left></vdr-ab-left>
                <vdr-ab-right>
                    <div>
                        <input type="number" [formControl]="amount" />
                        <button class="btn" (click)="request()">Request Exchange</button>
                    </div>
                </vdr-ab-right>
            </vdr-action-bar>
...
export class RequestExchange implements OnInit, CustomDetailComponent {
    ...
    request() {
        ...
        if (value) {
            this.dataService.mutate(RequestCreditExchangeDocument, { amount: value }).subscribe({
                next: data => {
